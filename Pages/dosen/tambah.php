<?php
session_start();
if (!isset($_SESSION['login'])) {
    header('location: ../login/login.php ');
}
require_once 'function.php';
require_once '../utility/conSQL.php';

if (isset($_POST['submit'])) {
    // ketikkan vardump untuk melihat index arraynya
    // var_dump($_POST);
    // var_dump($_FILES);

    if (tambah($_POST) > 0) {
        echo "
    <script>
    alert('data berhasil di tambah');
    document.location.href = 'dosen.php';
    </script>
    ";
    } else {
        echo "
    <script>
    alert('data gagal di tambah');
    </script>
    ";
    }
}
?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tambah Data Dosen</title>
    <link rel="stylesheet" type="text/css" href="../utility/index.css" />

</head>

<body>
    <!--
        encytype digunakan untuk membuat jalur untuk
        $_post untuk text dan $_file nya itu untuk file
        jadi jika di vardump $_post maka input bertype file tidak akan ditampilkan atau dikelola
    -->
    <?php
    include_once "../home/header.php";
    ?>
    <form action="" method="post" style="margin-left: 20px;">
        <table>
            <caption>
                <h1>Tambah Data Dosen</h1>
            </caption>
            <tr>
                <td><label for="dsn_nidn">NIDN</label></td>
                <td>:</td>
                <td><input type="text" id="dsn_nidn" name="dsn_nidn" required></td>
            </tr>
            <tr>
                <td><label for="dsn_nama">Nama</label></td>
                <td>:</td>
                <td><input type="text" id="dsn_nama" name="dsn_nama" required></td>
            </tr>
            <tr>
                <td><label for="dsn_alamat">Alamat</label></td>
                <td>:</td>
                <td>
                    <textarea name="dsn_alamat" id="dsn_alamat" cols="30" rows="5" required></textarea>
                </td>
            </tr>
            <tr>
                <td><label for="dsn_jenkel">Jenis Kelamin</label></td>
                <td>:</td>
                <td>
                    <input type="radio" id="dsn_jenkel" name="dsn_jenkel" value="Pria" required>Pria
                    <input type="radio" id="dsn_jenkel" name="dsn_jenkel" value="Wanita" required>Wanita
                </td>
            </tr>
            <tr>
                <td><label for="dsn_agama">Agama</label></td>
                <td>:</td>
                <td>
                    <select name="dsn_agama" id="dsn_agama">
                        <option value="Islam">Islam</option>
                        <option value="Kristen">Kristen</option>
                        <option value="Katolik">Katolik</option>
                        <option value="Buddha">Buddha</option>
                        <option value="Hindu">Hindu</option>
                        <option value="Konghucu">Konghucu</option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>
                    <label for=dsn_no_hp>No HP</label>
                </td>
                <td>:</td>
                <td>
                    <input type=text name=dsn_no_hp id=dsn_no_hp>
                </td>
            </tr>
            <tr>
                <td>
                    <button type="reset" name="reset" class="reset">Batal</button>
                </td>
                <td></td>
                <td>
                    <button type="submit" name="submit" class="submit">Kirim</button>
                </td>
            </tr>
        </table>
    </form>
    <?php
    include_once "../home/footer.php";
    ?>
</body>

</html>